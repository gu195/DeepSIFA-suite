...
"""
from functools import partial#Import partial from the standard library functools to freeze part of the parameters of the function and return a new function
from collections import OrderedDict#Typical use: Explicitly name each layer to nn.Sequential to maintain order and readability
...
import math
import numpy as np
from utils.Token_performer import Token_performer#Put common modules that will be used repeatedly in the code (such as attention implementation, convolution module, data processing function) into utils/. This is usually a directory (folder) created by the project author himself, specifically used to put some "utility (tool functions/tool modules)"
from utils.PRM import PRM
# cls and pos position codes have not been added yet
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=33, in_c=3, embed_dim=768, norm_layer=None):
        ...
        # self.proj = nn.Conv2d(in_c, embed_dim, kernel_size=patch_size, stride=patch_size)# To be modified to one-dimensional convolution
...
class SELayer(nn.Module):
    ...
    def forward(self, x):                   # [Batch, C, W] [Batch, 256, 256]
        ...
        # # Print results
        # print("x The first line of the first batch:")
        # print(x[0, 0, :])

        # print("\ny The first line of the first batch:")
        # print(y[0, 0, :])

        # print("\nresult The first line of the first batch:")
        # print(result[0, 0, :])
        return result
...
class BlockRC3(nn.Module):
    ...
    def forward(self, x):
        ...
        # Why can the C dimension of PRM_x be reduced to 64?
...
def vit_base_patch16_224(num_classes: int = 1000, tc_depth: int = 7):
    ...
    Weights ported from the official Google JAX implementation.
    Download link: https://pan.baidu.com/s/1zqb08naP0RPqqfSXfkB2EA
    Access code: eu9f
    """
