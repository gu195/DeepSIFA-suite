...
TEST_DATA_ROOT = os.path.join(AUTOMATION_ROOT, 'data', 'mlkl', 'test', 'v1')
...
def get_cfg(fold):
    ...
    parser.add_argument(
        '--weight_path',
        type=str,
        default=os.path.join(SCRIPT_DIR, 'checkpoints', 'fold{}'.format(fold), 'best_acc.pth'),
    )
    parser.add_argument(
        '--data_dir',
        type=str,
        default=os.path.join(TEST_DATA_ROOT, '归一化插值后npz_1024_高斯平滑1'),
    )
    parser.add_argument(
        '--val_csv_dir',
        type=str,
        default=os.path.join(TEST_DATA_ROOT, '337.csv'),
    )
    parser.add_argument(
        '--results_dir',
        default=os.path.join(SCRIPT_DIR, 'logs_test'),
        type=str,
        metavar='FILENAME',
        help='Output directory for test-set evaluation results.',
    )
