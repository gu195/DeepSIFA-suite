...
def dice_score(y_true, y_pred):
    ...
    Compute the score independently for each sample.
    """
...
def test(epoch, model, loader_eval, args):
    ...
    predicted_key2 = []
    predicted_key3 = []
    previous_labels = set()  # is used to store previously recorded label_4keys
    label_counts = {}  # Use a dictionary to record label_4keys and the corresponding number of occurrences
    ...
    with torch.no_grad():
        for input, _, label1, label2, label3, labelcls, name in tqdm(loader_eval):
            ...
            if (predicted == label_4keys).sum().item() == 4:
                ...
                label_4keys_flat = label_4keys_tolist[0]  # Get the first sublist
                label_4keys_tuple = tuple(label_4keys_flat)  # converted to tuple
                ...
                previous_labels.add(label_4keys_tuple)  # Add new label_4keys_tuple to the record collection

                # counts the number of occurrences of label_4keys
                if label_4keys_tuple in label_counts:
                    label_counts[label_4keys_tuple] += 1
                else:
                    label_counts[label_4keys_tuple] = 1

    # Print the number of occurrences of each label_4keys
...
if __name__ == '__main__':
    ...
    # output_str1 = 'Cortical bone interruption Test Results:\n'
    ...
    # write_results2txt(parse_config.results_dir, output_str1, 'cortical bone interruption')

    # output_str2 = 'Trabecular bone disruption Test Results:\n'
    ...
    # write_results2txt(parse_config.results_dir, output_str2, 'Trabecular bone disruption disorder')

    # output_str3 = 'Visible fracture line Test Results:\n'
    ...
    # write_results2txt(parse_config.results_dir, output_str3, 'Visible fracture line')
