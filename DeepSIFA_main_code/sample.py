...
def is_in_allowed_roots(path: str) -> bool:
    ...
    for root in ALLOWED_ROOTS:
        try:
            ...
            # will throw an exception if the drive letter is crossed, skip it directly.
            pass
    return False
...
def get_cfg():
    ...
    parser.add_argument(
        '--matlab_workdir',
        type=str,
        default=os.path.join(DATA_ROOT, 'Cache'),
    )
    parser.add_argument(
        '--matlab_m_source_dir',
        type=str,
        default=os.path.join(PROJECT_ROOT, 'CreateTrace'),
    )
    parser.add_argument(
        '--filename',
        type=str,
        default=os.path.join(DATA_ROOT, 'MLKL', 'film1_crop.tif'),
    )
    parser.add_argument(
        '--save_path',
        type=str,
        default=os.path.join(DATA_ROOT, 'MLKL', 'txt'),
    )
    ...
    parser.add_argument('--gaussFitWidth_values', type=int, default=6, help='Gaussian fit width values')#is the diameter
    ...
    parse_config = parser.parse_args()#Now parse_config is an object containing the properties of each parameter.
...
def delete_directories():
    directories_to_delete = [
        os.path.join(DATA_ROOT, 'Cache', TIF_NAME),
    ...
    for i, dir_path in enumerate(directories_to_delete):
            if os.path.exists(dir_path):
                ...
                if i == 1:
                   dir_path = os.path.normpath(dir_path)  # unified separator
...
# To be able to merge accurately, there should be no sudden changes in intensity. Cannot merge trajectories that are not a position into
# The png_group/id42_256_267_285 example still exposes a bug: two source items are processed, but three items disappear.
import re
import glob
import pandas as pd
...
def merge_and_delete_files(parse_config, threshold=50):
    """
    Merge trajectory pairs listed in ``message_group.csv``.

    Missing ``track*.csv`` or ``allFramesTrackInten*.csv`` files are reported
    and skipped. After a merge, the A intensity file is updated, the B files
    are removed with case-insensitive Track/track handling, both message CSV
    files are synchronized, and the action is recorded in ``merge_log.txt``.

    ``threshold`` is the maximum allowed frame gap between two trajectories.
    """
    # basic directory (consistent with the original script)
    ...
    for _, row in message_data.iterrows():
        ...
        parts = LIST.split()
        if len(parts) != 2:
            # only handles "two-by-two merge"; other complex groupings are skipped first
            skipped += 1
            continue

        # is parsed into two numbers
        try:
            X1, X2 = map(int, parts)
        except Exception:
            skipped += 1
        ...
        # The earlier time is A, the later time is B
        A1 = df_x1.iloc[0, 0]
        B1 = df_x2.iloc[0, 0]
        if A1 <= B1:
            A_num, B_num = X1, X2
        ...
        # Press A/B again to read (make sure the order is correct)
        ...
        # ===== 3) Whether the times are "connected" =====
        if abs(track_b_start - track_a_end) >= threshold:
            # does not meet the time splicing conditions, skip
            continue

        # ===== 4) allFrames file existence check and read =====
        ...
        # ===== 6) Delete the file of B (compatible with upper and lower case Track/track) =====
        _safe_remove(os.path.join(base_dir, f'allFramesTrackInten{B_num}.csv'))
        _safe_remove(os.path.join(base_dir, f'Track{B_num}.csv'))
        _safe_remove(os.path.join(base_dir, f'track{B_num}.csv'))

        # ===== 7) Synchronous update message.csv / message_group.csv / merge_log.txt =====
        try:
            ...
            if 'id' in mg.columns:
                mg = mg[mg['id'] != group_id]   # Simplified processing: discard the current merge group
...
def zhou_plot_points_on_image1(parse_config,params_suffix):#
    ...
    for _, row in message_data.iterrows():
        ...
        if matching_group.empty:
            ...
            ax.annotate(
                f"ID: {track_id}\n({x:.2f}, {y:.2f})",  # The displayed text includes track_id and coordinates
            ...
            for idx, group_id in enumerate(group_items):
                ...
                ax.annotate(
                    f"ID: {group_id}\n({x:.2f}, {y:.2f})",  # The displayed text content includes group_id and coordinates
                    xy=(x, y),# Starting position of the annotation arrow
                    xytext=(adjusted_x, adjusted_y),# Position of comment text
                    fontsize=1, color='white', ha='center', va='center',
                    bbox=dict(facecolor='red', alpha=0.8, edgecolor='none',pad=1),# Set the style of the text box
                    arrowprops=dict(arrowstyle="->", color="red", lw=1, shrinkA=0, shrinkB=0),# Set the style of the arrow
                )
    ...
    for _, row in message_data.iterrows():
        ...
        if matching_group.empty:
            ...
            for idx, group_id in enumerate(group_items):
                ...
                ax.annotate(
                    f"ID: {group_id}\n({x:.2f}, {y:.2f})",  # The displayed text includes group_id and coordinates
                    xy=(x, y),
...
def process_file_3(npz_file, save_path, params_suffix):
    ...
    # Adjust the spacing between sub-pictures and add hspace to increase the vertical spacing
    plt.subplots_adjust(hspace=0.3)  # Increase the interval between sub-pictures
...
def move_images_to_group(TIF_NAME, parse_config, params_suffix, save_path):
    ...
    for _, row in message_group_df.iterrows():
        # Extract id and LIST columns
        group_id = row['id']
        list_values = row['LIST'].split()  # Split string with spaces
...
if __name__ == "__main__":
    ...
    params_suffix = f"spotRadius{spotTrackingRadius}_guassWidth{gaussFitWidth}_Length{frameLength}_Gap{frameGap}"
    params_suffix = ''
    # Generate hyperparameter related SAVE_PATH
    SAVE_PATH = parse_config.save_path
    ...
    with open(os.devnull, 'w') as fnull:
        with redirect_stdout(fnull):
            ...
            merge_and_delete_files(parse_config, 50) # connection track
