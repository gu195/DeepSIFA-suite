...
class XrayDataset_train(Dataset):#For general training_2 categories
    def __init__(self,
                 args):
...
class XrayDataset_val(Dataset):#For general verification_2 categories
    def __init__(self,
                 args):
        df = pd.read_csv(args.val_csv_dir)
        self.npz_list = np.array(df['file_name'])
        self.label_list4 = np.array(df['label'])
        self.data_path = args.data_dir1
