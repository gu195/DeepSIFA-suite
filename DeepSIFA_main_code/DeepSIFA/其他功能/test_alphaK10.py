...
def get_cfg(fold):
    ...
    parser.add_argument('--data_dir', type=str, default='../data/alphaK10/第一批/归一化插值后npz_1024')
    parser.add_argument('--val_csv_dir', type=str, default='../data/alphaK10/第一批/109.csv')
    parser.add_argument('--results_dir', default='./logs_test_alphaK10/', type=str, metavar='FILENAME',
...
def test(epoch, model, loader_eval, loss_fn, fold):
    ...
    with torch.no_grad():
        # Initialization error prediction list
        ...
        for img, label, label_onehot, name in tqdm(loader_eval):
            ...
            # File name for recording prediction errors
            ...
            # records the file name of the prediction error and predicts the abnormality as normal
            if label == 1  and preds == 0:
                name_message = [name[0], label.item(), preds.item()]
                nameFN.append(name_message)


            # records the predicted correct file name, and the prediction is 1
    ...
    return evaluation_metrics, nameFP, nameFN, nameTP1, nameTP2, nameTN, allscore
...
if __name__ == '__main__':
    ...
    for fold in range(1,6):
        ...
        log_path_foldTP2 = log_path + 'fold{}/TP2'.format(fold)
        os.makedirs(log_path_foldTP2, exist_ok=True)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            start = time.time()
            eval_metrics, nameFP, nameFN, nameTP1, nameTP2, nameTN, allscore = test(epoch, model, loader_eval, cls_loss2, fold)
            ...
            # Traverse the correct_preds1 list and copy the corresponding files to the target directory
            ...
            for filename in nameTP2:
                if filename[0].endswith('.npz'):
                    source_file = os.path.join(parse_config.data_dir + '/png1', filename[0].replace('.npz', '.png'))
                    target_file = os.path.join(log_path_foldTP2, filename[0].replace('.npz', '.png'))
                    shutil.copy2(source_file, target_file)
                    with open(log_path_foldTP2 + '/file.csv', 'a', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        writer.writerow([filename[0]])
    ...
    #----------------Save the prediction errors to ./logs_val verification/FN.txt ----------------------------------------
    ...
    #----------------Save the prediction errors to ./logs_val verification/FP.txt ----------------------------------------
    ...
    #----------------Save the predicted correct 1 to ./logs_val verification/TP1.txt ----------------------------------------
    ...
    with open(os.path.join(parse_config.results_dir, 'TP2.txt'), 'w') as file:
        for item in global_nameTP2:
            for item_sub in item:
                file.write(f'{item_sub}\n')
    # --------------------------------------------------------------------------------------------
                
    #----------------Save the prediction errors to ./logs_val verification/TN.txt ----------------------------------------
