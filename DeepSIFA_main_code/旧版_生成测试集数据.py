...
def process_file_2(npz_file):# Visualized normalized interpolation npz_{NUM_dimension}_Gaussian smoothing {SIGMA}
    # source_dir = os.path.join('.', 'data', str(DATA), 'test', f'v{NUM}', f'npz_{NUM_dimension}_Gaussian smoothing {SIGMA}' after normalized interpolation)
    # file_path = os.path.join(source_dir, filename)
...
def process_file_3(npz_file):#Visualize raw data
    # defines the path of npz file
    directory = os.path.join('.', 'data', str(DATA), 'test', f'v{NUM}', '原始数据')
    source_dir = os.path.join('.', 'data', str(DATA), 'test', f'v{NUM}', f'归一化插值后npz_{NUM_dimension}_高斯平滑{SIGMA}')
    os.makedirs(os.path.join(source_dir, 'png'), exist_ok=True)
