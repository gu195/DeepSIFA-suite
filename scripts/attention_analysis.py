...
from monoxtract.model import Attention, load_model_from_checkpoint  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=REPOSITORY_ROOT / "checkpoints" / "best_acc.pth",
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=REPOSITORY_ROOT / "data/mlkl/validation/labels.csv",
        help="CSV with file_name,label columns.",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=REPOSITORY_ROOT / "data/mlkl/validation/traces",
        help="Directory containing 1024-point NPZ files.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPOSITORY_ROOT / "outputs/attention_results",
    )
...
def discover_dataset(
    ...
    if csv_path is None:
        matches = sorted(REPOSITORY_ROOT.rglob("labels.csv"))
        if not matches:
            raise FileNotFoundError(
                "Could not find labels.csv; pass --csv and --data-dir explicitly."
            )
        csv_path = matches[0]
...
def build_model(checkpoint: Path, device: torch.device) -> nn.Module:
    model = load_model_from_checkpoint(checkpoint, device, strict=True)
    model.eval()
    return model
